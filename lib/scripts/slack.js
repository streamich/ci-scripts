...
const defaultMessage = ({PROJECT_NAME, BUILD_VERSION, BUILD_URL, CI_NAME, PROJECT_URL,
    ...
    if (IS_PR) {
        msg += ` :fingerscrossed: <${BUILD_PR_URL}|\`/pull/${BUILD_PR_NUM}\`>`;
    }

    if (BUILD_URL) {
        msg += ` :fingerscrossed: <${BUILD_URL}|${CI_NAME}>`;
    }

    msg += ` :fingerscrossed: ${'*`' + BUILD_VERSION + '`*'}`;
...
const slack = async (ci, params) => {
    ...
    log(`Posted to Slack: ${ci.BUILD_VERSION}`);
